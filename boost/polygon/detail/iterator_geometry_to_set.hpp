...
public:
  ...
  iterator_geometry_to_set(const rectangle_type& rectangle, direction_1d dir,
                           orientation_2d orient = HORIZONTAL_, bool is_hole = false, bool = false, direction_1d = CLOCKWISE) :
...
public:
  ...
  iterator_geometry_to_set(const polygon_type& polygon, direction_1d dir, orientation_2d orient = HORIZONTAL_, bool is_hole = false, bool winding_override = false, direction_1d w = CLOCKWISE) :
  ...
  inline void evaluate_() {
    ...
    } else if(pts[0].get(HORIZONTAL_) != pts[1].get(HORIZONTAL_)) {
      vertex_.second.second = -1;
    } else if(pts[0].get(VERTICAL_) != pts[1].get(VERTICAL_)) {
      vertex_.second.second = 1;
    } else {
      vertex_.second.second = 0;
    }
